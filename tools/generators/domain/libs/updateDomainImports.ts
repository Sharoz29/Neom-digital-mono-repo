...
export function updateDomainImports(
  tree: Tree,
  ...
  const toInsertModuleStatement = ` ${names(options.name).className}DomainModule,
  `;
  ...
  }DomainModule} from './${names(options.name).fileName}-domain/${
  ...
  const newContents = tsquery.replace(
    ...
    (node) => {
      let modifiedNode = node.getFullText();
      if (
        modifiedNode.includes(`{${names(options.name).className}DomainModule}`) ||
        alreadyImported
      ) {
