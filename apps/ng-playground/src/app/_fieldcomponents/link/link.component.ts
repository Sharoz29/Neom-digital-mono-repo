...
export class LinkComponent implements OnInit {
  ...
  showLabel$ = false;
  linkStyleClass$ = "mat-button pega-link";
  reference!: string;
  tooltip$!: string;
  ...
  ngOnInit() {
    ...
    const bFormat = this.fieldComp.control.modes[1].controlFormat ? this.fieldComp.control.modes[1].controlFormat : "";
  ...
  linkClick(e: any) {
    ...
    if (this.fieldComp.disabled != true) {
      ...
      if (this.fieldComp.control.modes[0] && this.fieldComp.control.modes[0].linkType === "url") {
        ...
        // eslint-disable-next-line no-useless-escape
        const url = this.fieldComp.control.modes[0].linkData.replace( /\"/gi, "");;
