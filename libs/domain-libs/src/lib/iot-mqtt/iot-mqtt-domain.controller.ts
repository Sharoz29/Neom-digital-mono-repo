...
export class IotMqttDomainController {
   ...
   * @returns {Promise<any>} The detailed information of the device from Cumulocity.
  ...
  async fetchDeviceDetailsFromCumulocity({ deviceID }: { deviceID: string }) {
    this.logger.log(`Fetching Device Data from ID ${deviceID}`);
    try {
      const result = await this._iotMqttDomainService.fetchDeviceDetailsFromCumulocity(deviceID);
      return result;
    } catch (error: any) {
      this.logger.error(`Unable to fetch Device: ${deviceID}, Error: ${error.message}`);
      throw error;
    }
  }
   ...
   * @returns {Promise<any>} The result of the registration and subscription operation.
  ...
  async registerAndSubscribeDeviceToCumulocity({ topic }: { topic: string }) {
    this.logger.log(`Fetching Device topic: ${topic}`);
    try {
      const result = await this._iotMqttDomainService.registerAndSubscribeDevices(topic);
      return result;
    } catch (error: any) {
      this.logger.error(`Unable to fetch Device: ${topic}, Error: ${error.message}`);
      throw error;
    }
  }
}
