...
export class IotMqttDomainController {
  ...
  async publishMessage(
    ...
    try {
      return await this._iotMqttDomainService.publish(topic, message);
    } catch (error: any) {
  ...
  async publishMessageFromCumulocity(
    ...
    try {
      return await this._iotMqttDomainService.publishFromCumulocity(
        topic,
        message
      );
    } catch (error: any) {
  ...
  async getCachedMessage(@Param('topic') topic: string): Promise<string> {
    try {
      return await this._iotMqttDomainService.getCachedMessage(topic);
    } catch (error: any) {
      throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async subscribeToTopic(@Param('topic') topic: string): Promise<string> {
    try {
      this._iotMqttDomainService.subscribe(topic, (message) => {
